const validate = {
  ...
  checkIfString: (inp) => {
    if (typeof inp !== "string" || inp.trim().length === 0) throw "Error: Input parameter must be non-empty string";
  },

  checkIfPositiveNumber: (inp) => {
    if (typeof inp !== "number" || inp === NaN) throw "Error: Input parameter must be a positive number";
  ...
  checkIfValidURL: (inp) => {
    ...
    if (inp.substring(strLength - 4, strLength) !== ".com") throw "Error: Invalid URL";
    if (inp.slice(11, -4).length < 5) throw "Error: Invalid URL";
    let site = inp.slice(11, -4);
    if (/^[a-zA-Z0-9]+([\-_\.][a-zA-Z0-9]+)*$/.test(site) === false) throw "Error: Invalid URL";   // https://javascript.plainenglish.io/check-if-string-is-alphanumeric-in-javascript-e325caa3ee
  },

  checkIfValidArray: (inp) => {
    if (typeof inp !== 'object' || Array.isArray(inp) !== true) throw "Error: Input parameter must be an array";
  ...
  checkIfValidDate: (inp) => {
    ...
    if (parseInt(inp.substring(0, 2)) < 1 || parseInt(inp.substring(0, 2)) > 12) throw "Error: Invalid date"; //for checking month
    let month = parseInt(inp.substring(0, 2));
    if (parseInt(inp.substring(3, 5)) < 1 || parseInt(inp.substring(3, 5)) > days[month - 1]) throw "Error: Invalid date"; //for checking day
    if (parseInt(inp.substring(6)) > todayDate.getFullYear()) throw "Error: Invalid date"; //for checking year
    if (inpDate > todayDate) throw "Error: Invalid Date"; // for checking if date in future
  },

  checkIfBoolean: (inp) => {
    if (typeof inp !== 'boolean') throw "Error: Input parameter must be boolean";
  },
  ...
  checkIfValidRating: (inp) => {
    ...
    if (!Number.isInteger(inp)) {
      let temp = inp.toString().split('.');
      if (temp[1].length > 1) throw "Error: Only one decimal points allowed for rating";
    }
  },
